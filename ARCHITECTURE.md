...
公開: トップ https://mikushiba.github.io/kanji-quiz/ ／ 同じ読み `…/onaji/` ／ 送りがな `…/okurigana/` ／ 部首 `…/bushu/`
...
| 部首を足す/直す | `bushu/index.html` の `RADICALS`（`{c,pos,mean,ks:[例字…]}`。例字は全て `KANJI` に実在・1字は1部首のみ。`node validate.mjs` で検証） |
